...
install -d -m 0700 -o "$OUTPUT_UID" -g "$OUTPUT_GID" "$key_dir"
if [ ! -f "$SSH_KEY" ]; then
	ssh-keygen -q -t ed25519 -N '' -C raphael-linux-bringup -f "$SSH_KEY"
	chown "$OUTPUT_UID:$OUTPUT_GID" "$SSH_KEY" "$SSH_KEY.pub"
	chmod 0600 "$SSH_KEY"
...
# package bootstrap.
write_rootfs_configuration "$root" "$SSH_KEY.pub"
